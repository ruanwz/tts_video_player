...
def tts():
    ...
    try:
        ...
        engine = data.get('engine', 'edge') # 默认 edge
        
        logger.info(f"TTS Request: engine='{engine}', text='{text[:20]}...', voice='{voice}', rate='{rate}'")
        ...
        if engine == 'google':
            try:
                # voice 参数在这里作为语言代码 (e.g., 'zh-cn', 'en')
                lang = voice 
        ...
        try:
            ...
            logger.error(f"Edge TTS failed: {e}, trying gTTS fallback...")
            try:
                # Fallback to gTTS
                # Map Edge voice to gTTS lang code
                lang = 'zh-cn'
                if 'en-' in voice:
                    lang = 'en'
                elif 'ja-' in voice:
                ...
                tts = gTTS(text=text, lang=lang)
                tts.save(str(output_file))
                logger.info(f"gTTS fallback successful")
...
def download_video():
    ...
    try:
        ...
        # yt-dlp配置
        ydl_opts = {
            'format': 'bestvideo[height<=720]+bestaudio/best[height<=720]',  # 限制最高720p
...
@app.route('/api/match-subtitle', methods=['GET'])
def match_subtitle():
    """匹配视频的字幕文件"""
    try:
        ...
        video_path = Path('video') / video_filename
        ...
        priorities = [
            ...
            f"{base_name}.zh-Hans.vtt",
            f"{base_name}.en.srt",
            f"{base_name}.en.vtt"
        ]
        
        for sub_name in priorities:
            sub_path = video_dir / sub_name
            if sub_path.exists():
                return jsonify({
                    'found': True,
                    'name': sub_name,
                    'url': f"/video/{sub_name}",
                    'lang': 'zh' if 'zh' in sub_name else 'en'
                })
                
        return jsonify({'found': False})
        
    except Exception as e:
        logger.error(f"匹配字幕失败: {str(e)}")
        return jsonify({'error': str(e)}), 500
